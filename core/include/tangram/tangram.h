...
public:
    ...
    // Set an explicit draw group for a marker object; _drawGrpPath is the absoulte path to a draw
    // group in the scene being loaded. Draw rules specified in this draw group will will be applied
    // for marker styling.
    // returns true if the marker ID was found and successfully updated, otherwise returns false.
    bool markerSetLayerDrawGroup(MarkerID _marker, const char* _drawGrpPath);
